...
def create_dim_table_from_column(
    ...
    try:
        ...
        result_df = (
            df.select(
                explode(split(col(column_name), delimiter + r"\s*")).alias(value_column)
            )
        ...
        logger.info(
            f"Successfully created dimension table with {result_df.count()} records."
        )
        return result_df
...
def create_bridge_table(
    ...
    try:
        logger.info(
            f"Creating bridge table between fact: {fact_list_col} and dimension: {dim_value_col}"
        )
        ...
        joined_df = exploded_df.join(
            dim_df, exploded_df[fact_list_col] == dim_df[dim_value_col], "inner"
        )
        ...
        logger.info(
            f"Successfully created bridge table with {bridge_df.count()} records."
        )
        return bridge_df
...
def process_tables_incrementally(df):
    ...
    try:
        ...
        for col_name, bridge_table in create_bridge_tables_generator(
            df, dim_tables, dim_specs
        ):
